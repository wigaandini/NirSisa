...
// Synchronized with backend normalizer.py STAPLE_INGREDIENTS
const STAPLE_SET = new Set([
  ...
  "saus tomat", "bawang bombay", "merica", "merica bubuk",
  "minyak", "kecap", "saus",
]);

const checkIsStapleClean = (cleanName: string): boolean => {
  if (!cleanName) return false;
  const lower = cleanName.toLowerCase().trim();
  return STAPLE_SET.has(lower);
};

// Legacy staple check for raw text fallback
const STAPLE_INGREDIENTS_LEGACY = [
  ...
  "bumbu", "tumisan", "saos", "sambal", "minyak sayur",
  "garlic", "daun bawang",
];

const checkIsStapleLegacy = (text: string): boolean => {
  if (!text) return false;
  const normalized = text.toLowerCase();
  const noNumbers = normalized.replace(/[0-9/.,]/g, " ");
  const noUnits = noNumbers.replace(
    /\b(ml|gr|g|kg|l|liter|sdt|sdm|tsp|tbsp|ons|cc|bungkus|bks|buah|pcs|batang|btg|siung|ikat|ruas|lembar|cm|bh|secukupnya|utk|untuk|taburan|iris|daun|cincang|geprek|halus|kasar|sesuai|selera|secukup nya|sedikit)\b/g,
    " "
  );
  const cleanContent = noUnits.trim();
  if (!cleanContent) return false;
  const tokens = cleanContent.split(/\s+/);
  const firstToken = tokens[0];
  const combinedToken = tokens.length > 1 ? `${tokens[0]} ${tokens[1]}` : tokens[0];
  return STAPLE_INGREDIENTS_LEGACY.some((staple) => {
...
const UNIT_ALIASES: Record<string, string> = {
  gr: "g", gram: "g", grams: "g", ons: "ons",
  kg: "kg", kilogram: "kg",
  ml: "ml", l: "l", liter: "l",
  sdt: "tsp", sdm: "tbsp", tsp: "tsp", tbsp: "tbsp",
  butir: "pcs", buah: "pcs", pcs: "pcs", pc: "pcs",
  batang: "batang", btg: "batang",
  siung: "siung", ikat: "ikat", ruas: "ruas",
  bungkus: "pcs", bks: "pcs", cc: "ml",
  ekor: "pcs", lembar: "pcs", potong: "pcs",
  bonggol: "pcs", biji: "pcs", iris: "pcs",
};
...
const STOPWORDS = new Set([
  "gram", "gr", "kg", "ons", "ml", "l", "liter", "sdt", "sdm",
  "tsp", "tbsp", "butir", "buah", "pcs", "pc", "batang", "siung",
  "ikat", "lembar", "bungkus", "cm", "secukupnya", "sesuai", "selera",
  "iris", "tipis", "halus", "kasar", "potong", "buang", "kulit",
  "kerasnya", "bahan", "dan", "atau", "geprek", "memarkan", "haluskan",
  "cincang", "sendok", "makan", "teh", "gelas", "cup", "cc",
  "cuci", "bersih", "seduh", "dirajang", "matang", "rebus", "air",
  "panas", "tiriskan", "suir", "suwir", "bh",
]);

const SYNONYMS: Record<string, string> = {
  toge: "tauge", kecambah: "tauge", rawit: "cabai",
  cabe: "cabai", telor: "telur", bamer: "bawang", baput: "bawang",
};
...
type IngredientMatchStatus = "safe" | "partial" | "uncertain" | "missing";
...
useEffect(() => {
  ...
  // v4: use pre-cleaned ingredients_cleaned + quantity fields
  // Fallback: legacy raw parsing from recipe.ingredients
  // ─────────────────────────────────────────────────────────────────────────
  const ingredientList: IngredientView[] = useMemo(() => {
    const hasCleanData = !!(recipe.ingredients_cleaned && recipe.ingredients_cleaned.trim());

    // Prepare items: { displayText, cleanName, qtyStr }
    let items: { displayText: string; cleanName: string; qtyStr: string }[];

    if (hasCleanData) {
      // v4 path: split comma-separated cleaned data
      const names = recipe.ingredients_cleaned
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
      const qtys = (recipe.quantity || "")
        .split(",")
        .map((s) => s.trim());
      ...
      // Legacy fallback: parse raw ingredients text
    ...
    return items.map((item, idx) => {
      const isStaple = hasCleanData
        ? checkIsStapleClean(item.cleanName)
        : checkIsStapleLegacy(item.displayText);

      // Extract requested amount from quantity string (v4) or raw text (legacy)
      const requested = extractRequestedAmount(item.qtyStr);
      ...
      if (isStaple) {
        return {
          ...
          quantityToUse: null,
          status: "safe" as IngredientMatchStatus,
          helperText: "Bahan pendukung (Asumsi Tersedia)",
        };
      }

      // Match clean ingredient name against user inventory
      const matched = pickBestInventoryMatch(item.cleanName, inventory);
      ...
      if (!matched) {
        return {
          ...
          requestedUnit: requested.unit,
          quantityToUse: null,
          status: "missing" as IngredientMatchStatus,
      ...
      if (quantityToUse === null) {
        return {
          ...
          requestedUnit: requested.unit,
          quantityToUse: null,
          status: "partial" as IngredientMatchStatus,
      ...
      return {
        ...
        quantityToUse,
        status: "safe" as IngredientMatchStatus,
        helperText: `Akan pakai ${quantityToUse.toFixed(2)} ${capitalizeEachWord(
          matched.unit || ""
        )} dari stok ${matched.quantity} ${capitalizeEachWord(matched.unit || "")}`,
      };
    });
  }, [recipe.ingredients_cleaned, recipe.quantity, recipe.ingredients, inventory]);
